...
PINECONE_NAMESPACE = "plagiarism-checker-articles"
...
def create_pinecone_index():
    pinecone.create_index(name=PINECONE_INDEX_NAME, dimension=300)
...
def upload_items(data):
    ...
    pinecone_index.upsert(vectors=items_to_upload, namespace=PINECONE_NAMESPACE)
...
def query_pinecone(originalContent):
    ...
    query_vectors = model.encode(query_content).tolist()

    query_results = pinecone_index.query(namespace=PINECONE_NAMESPACE, vector=query_vectors, top_k=10)
    print(query_results)
    ...
    for r in res:
        ...
        results_list.append({
            "id": r.id,
            "title": titles_mapped[int(r.id)],
            "publication": publications_mapped[int(r.id)],
            "score": r.score,
        })
